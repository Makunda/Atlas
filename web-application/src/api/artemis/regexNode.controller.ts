...
export class RegexNodeController {
  ...
  public static async addRegexNode(item:ApiRegexNode) : Promise<ApiRegexNode> {
    const url =
    RegexNodeController.API_BASE_URL + "/api/artemis/regexes/add";
    ...
    try {
      ...
      if (res.status == 200) {
        ...
        throw new Error(`Failed to add a Regex Node. Status (${res.status}) : Error : ${res.data}`);
      }
    } catch (error) {
      console.error(
  ...
  public static async updateRegexNode(item:ApiRegexNode) : Promise<ApiRegexNode> {
    const url =
    ...
    try {
      ...
      if (res.status == 200) {
        ...
        throw new Error(`Failed to update a Regex Node. Status (${res.status}) : Error : ${res.data}`);
      }
    } catch (error) {
      console.error(
  ...
  public static async getAllNode() : Promise<ApiRegexNode[]> {
    const url =
    RegexNodeController.API_BASE_URL + "/api/artemis/regexes/all";
    ...
    try {
      ...
      if (res.status == 200) {
        ...
        throw new Error(`Failed to retrieve the list of Regex Nodes. Status (${res.status}) : Error : ${res.data}`);
      }
    } catch (error) {
      console.error(
  ...
  public static async testRegex(id:number) : Promise<number> {
    ...
    try {
      ...
      if (res.status == 200) {
        ...
        throw new Error(`Failed to test the Regex Node. Status (${res.status}) : Error : ${res.data}`);
      }
    } catch (error) {
      console.error(
  ...
  public static async deleteRegexNode(id:number) : Promise<boolean> {
    const url =
    RegexNodeController.API_BASE_URL + `/api/artemis/regexes/${id}`;
    ...
    try {
      ...
      if (res.status == 200) {
        ...
        throw new Error(`Failed to delete the Regex Node. Status (${res.status}) : Error : ${res.data}`);
      }
    } catch (error) {
      console.error(
