...
class DataCallGraphController {
    ...
    public getInsightsUnmaskedDataGraphs = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try {
            const applicationName = String(req.params.application);
            const insights: Record<string, unknown> = await this.dataCallGraphService.getInsightsUnmaskedDataGraph(applicationName);
    ...
    public getBatchMaskedDataGraph = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try {
            ...
            const dataGraphs: DataCallGraph[] = await this.dataCallGraphService.getBatchMaskedDataGraph(applicationName, startIndex, endIndex, sort, sortDesc);
            res.status(200).json({data: dataGraphs, message: 'Batch'});
    ...
    public pinDataGraph = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        ...
        try {
            ...
            const datagraph: DataCallGraph = await this.dataCallGraphService
                .pinDataGraph(applicationName, dataCallGraphID, prefix);
    ...
    public unpinDataGraph = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        ...
        try {
            const datagraph: DataCallGraph = await this.dataCallGraphService
                .unpinDataGraph(applicationName, dataCallGraphID, prefix);
            res.status(200).json({data: datagraph, message: 'Renamed DataGraph'});
    ...
    public renameDataGraph = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        ...
        try {
            const datagraph: DataCallGraph = await this.dataCallGraphService.renameDataGraph(applicationName, dataCallGraphID, datagraphName);
            res.status(200).json({data: datagraph, message: 'Renamed DataGraph'});
    ...
    public unmaskDataGraph = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        ...
        try {
            const datagraph: DataCallGraph = await this.dataCallGraphService.unmaskDataGraph(applicationName, dataCallGraphID);
            res.status(200).json({data: datagraph, message: 'DataGraph'});
        } catch (error) {
            logger.error(`Failed to unmask datagraph with id:'${dataCallGraphID}' in application with name '${applicationName}.`, error);
            next(error);
        }
    };
    ...
    public maskObjectByTerms = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        ...
        try {
            ...
            logger.error(`Failed to mask DataGraph with certain terms inside it in application with name ${applicationName}.`, error);
            next(error);
        }
    };
