...
export class FrameworkAssistantManager {
  ...
  public async applyLevelTagOnObjects(category: string) {
    ...
    if (res.records.length > 0 && res.records[0].get("count") != 0) {
      logger.info(
        `The assistant regrouped ${res.records[0].get("count")} nodes under their level for category ${category}`
      );
    }
  }
  ...
  public async applyModuleTagOnObjects(category: string) {
    ...
    if (res.records.length > 0 && res.records[0].get("count") != 0) {
      logger.info(
        `The assistant regrouped ${res.records[0].get("count")} nodes under their module for category ${category}`
      );
    }
  }
  ...
  public async applyArchitectureTagOnObjects(category: string) {
    ...
    if (res.records.length > 0 && res.records[0].get("countObj") != 0) {
      logger.info(
        `The assistant regrouped ${res.records[0].get(
          "countObj"
        )} nodes under their architecture for category ${category}.\n ${res.records[0].get(
          "countOther"
  ...
  private async fetchResources() {
    try {
      ...
      if (!this.artemisCategoryProperty || this.artemisCategoryProperty.length == 0)
