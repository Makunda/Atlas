...
export default class InstallationService {
        ...
        try {
            ...
            while (directoryStack.length != 0) {
                try {
                    ...
                    let toSkip = false;
                    for(const avoid of this.TO_AVOID_FOLDER) if(filepath.includes(avoid)) { toSkip = true; };
                    if(toSkip) continue;
    ...
    public setImagingFolder(newPath: string): void {
        if(!fs.existsSync(newPath))
            throw new Error(`The path ${newPath} does not exist. Please specify a valid path.`)
        if(!fs.existsSync(newPath  + InstallationService.RELATIVE_NEO4J_PLUGINS))
            throw new Error(`The path ${newPath  + InstallationService.RELATIVE_NEO4J_PLUGINS} does not exist. Please specify a valid path.`)
        if(!fs.existsSync(newPath + InstallationService.RELATIVE_NGINX_ICONS))
            throw new Error(`The path ${newPath + InstallationService.RELATIVE_NGINX_ICONS} does not exist. Please specify a valid path.`)
