...
class UseCaseService {
   ...
   * Get the version of Artemis
   */
  public async checkValidity(request: string, returnValue: string): Promise<boolean> {
    try {
      const req = "CALL paris.cypher.test.request($request, $returnValue)";
      const params = { request: request, returnValue: returnValue };

      const results: QueryResult = await this.neo4jAl.executeWithParameters(req, params);
  ...
  public async getAttachedUseCases(id: number): Promise<IUseCase[]> {
    try {
      ...
      logger.error("Failed to verify the retrieve the list of attached use cases", err);
      throw new HttpException(500, "Internal error");
    }
  }
  ...
  public async addUseCase(useCase: IUseCase): Promise<IUseCase> {
    try {
      const request = "CALL paris.cases.create($title, $description, $categories, $active, $selected)";
   ...
   */
  public async attachUseCase(idParent: number, idChild: number): Promise<boolean> {
    try {
   ...
   * @param idChild Id of the child
   */
  public async detachUseCase(idParent: number, idChild: number): Promise<boolean> {
    try {
   ...
   */
  public async createAndAttach(useCase: IUseCase, idParent: number): Promise<IUseCase> {
