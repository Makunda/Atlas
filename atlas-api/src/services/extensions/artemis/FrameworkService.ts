...
import CategoryService from "@services/extensions/artemis/CategoryService";
...
class FrameworksService {
   ...
   */
  public async findFrameworkbyName(name: string, internalType?: string): Promise<Framework> {
    try {
      let val: QueryResult;
  ...
  public async searchFrameworkByName(name: string): Promise<Framework[]> {
    const res: QueryResult = await this.neo4jAl.executeWithParameters("CALL artemis.api.find.framework.name.contains($name, 10);", { name: name });
  ...
  public async findFrameworkbyNameAndType(name: string, internalType: string): Promise<Framework> {
    ...
    try {
      ...
      logger.error("An internal error occurred in FrameworksServices::findFrameworkbyNameAndType ", err);
      throw new HttpException(500, "Internal error");
    }
  }

  /** Merge the framework */
  public async mergeFramework(frameworkData: CreateFrameworkDto): Promise<Framework> {
    // Clean Framework
    ...
    try {
      ...
      logger.error("An internal error occurred in FrameworksServices::mergeFramework ", err);
      throw new HttpException(500, "Internal error");
    }
  }
   ...
   * @param frameworkData
   */
  public async updateFramework(oldName: string, oldInternalType: string[], frameworkData: CreateFrameworkDto): Promise<boolean> {
    ...
    if (inBase == null) throw new HttpException(404, `Framework with name ${frameworkData.name} does not exist.`);
    ...
    try {
      ...
      logger.error("An internal error occurred in FrameworksServices::updateFrameworks", err);
      throw new HttpException(500, "Internal error");
    }
  }
  ...
  public async updateFrameworkById(frameworkData: CreateFrameworkDto): Promise<boolean> {
    ...
    try {
      ...
      logger.error("An internal error occurred in FrameworksServices::updateFrameworksById", err);
      throw new HttpException(500, "Internal error");
    }
  }
  ...
  public async getToValidateFrameworks(): Promise<Framework[]> {
    const res: QueryResult = await this.neo4jAl.execute("CALL artemis.api.get.framework.to.validate();");
  ...
  public async getFrameworksinternalType(): Promise<string[]> {
    const res: QueryResult = await this.neo4jAl.execute("CALL artemis.api.get.framework.internalType()");
  ...
  public async getDuplicateFrameworks(): Promise<Framework[]> {
    const res: QueryResult = await this.neo4jAl.execute("CALL artemis.api.get.framework.duplicates()");
  ...
  public async deleteFrameworkById(id: number): Promise<boolean> {
    ...
    try {
      ...
      logger.error("An internal error occurred in FrameworksServices::deleteFrameworkById", err);
      throw new HttpException(500, "Internal error");
    }
  }
  ...
  public async toggleFrameworkTypeById(id: number): Promise<boolean> {
    ...
    try {
      ...
      logger.error("An internal error occurred in FrameworksServices::toggleFrameworkTypeById", err);
      throw new HttpException(500, "Internal error");
    }
  }
}
