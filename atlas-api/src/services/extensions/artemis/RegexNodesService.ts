...
function convertRecordToRegexNode(record: any): RegexNode {
  const regex: RegexNode = {
    ...
    parentId: record.get("parentId") != null ? int(record.get("parentId")).toNumber() : -1,
  };
...
class RegexNodesService {
  ...
  public async getAllNodesAsTree(): Promise<RegexNode[]> {
    ...
    try {
      ...
      logger.error("An internal error occurred in RegexNodeService::getAllNodesAsTree ", err);
  ...
  public async addRegexNode(node: CreateRegexNodeDto): Promise<RegexNode> {
    ...
    try {
      ...
      logger.error("An internal error occurred in RegexNodeService::addRegexNode ", err);
      throw new HttpException(500, "Internal error");
    }
  }
  ...
  public async updateRegexNode(node: CreateRegexNodeDto): Promise<RegexNode> {
    ...
    try {
      ...
      logger.error("An internal error occurred in RegexNodeService::updateRegexNode ", err);
      throw new HttpException(500, "Internal error");
    }
  }
