...
class GroupService {
   ...
   */
  public async attachToUseCase(idUseCase: number, idGroup: number): Promise<boolean> {
    try {
      const request = "CALL paris.groups.attach.to.case($idUseCase, $idGroup);";
      const params = { idUseCase: idUseCase, idGroup: idGroup };
      const results: QueryResult = await this.neo4jAl.executeWithParameters(request, params);
   ...
   * @returns
   */
  public async executeListGroup(application: string, idList: number[], executionType: string): Promise<number> {
    try {
      const request = "CALL paris.groups.execute.list($application, $idList, $executionType);";
      const params = {
   ...
   * @param application Name of the application
   */
  public async forecastAll(application: string): Promise<IGroupResult[]> {
    try {
      const request = "CALL paris.groups.forecast.all($application);";
      const params = {
        application: application,
      };
   ...
   */
  public async detachFromUseCase(idUseCase: number, idGroup: number): Promise<boolean> {
    try {
      const request = "CALL paris.groups.detach.from.case($idUseCase, $idGroup);";
      const params = { idUseCase: idUseCase, idGroup: idGroup };
      const results: QueryResult = await this.neo4jAl.executeWithParameters(request, params);
   ...
   */
  public async createAndAttach(group: IGroup, idUseCase: number): Promise<IGroup> {
   ...
   */
  public async changeParent(idGroup: number, idParentOld: number, idParentNew: number): Promise<void> {
