...
class DataCallGraphService {
     ...
     */
    public async getBatchDataGraph(application: string, start: number, end: number,
                                     filter?: Record<string, unknown>,
                                     sort?: string,
                                     sortDesc?: boolean): Promise<DataCallGraph[]> {
        ...
        try {
            ...
            ORDER BY ${DataCallGraphService.getSortParameter("tran", sort)} ${DataCallGraphService.getSortDesc(sortDesc)} 
            ...
            logger.error(
                `Failed to get catch of DataGraph for application ${application}.`,
                err
            );
     ...
     */
    public async renameDataGraph(application: string, idDataGraph: number, newName: string):
        ...
        try {
            ...
            const result = await this.neo4jAl.executeWithParameters(req,
                {"idTran": int(idDataGraph), newName: newName});
            if (!result || result.records.length == 0) return null;
     ...
     * @param sortDesc Direction of the sort
     */
    public async getBatchMaskedDataGraph(application: string, start: number,
        ...
        try {
            ...
            logger.error(
                `Failed to get the list of DataGraph for application ${application}.`,
                err
            );
            throw new HttpException(500, "Internal error");
        }
    ...
    public async maskDataGraphWithFilter(application: string, filter: Record<string, unknown>): Promise<number> {
        try {
            ...
            logger.error(
                `Failed to mask a DataGraph for application ${application}.`,
                err
            );
            throw new HttpException(500, "Internal error");
        }
    ...
    public async unmaskDataGraph(application: string, idDataGraph: number): Promise<DataCallGraph> {
        ...
        RETURN t as node;`;
            ...
            logger.error(
                `Failed to un-mask a DataGraph for application ${application}.`,
                err
            );
            throw new HttpException(500, "Internal error");
        }
    ...
    public async unmaskAllDataGraph(application: string): Promise<boolean> {
        ...
        RETURN t as node;`;
            ...
            logger.error(
                `Failed to un-mask all DataGraph in application ${application}.`,
                err
            );
            throw new HttpException(500, "Internal error");
        }
