...
export default class HighlightCloudBlockerService {
  ...
  public async processExcel(application: string, path: string): Promise<CloudBlocker[]> {
    ...
    if (!workbook.worksheets.map(x => x.name).includes(HighlightCloudBlockerService.CLOUD_WORKSHEET)) {
      throw new Error(`Excel report is not correct. Failed to find '${HighlightCloudBlockerService.CLOUD_WORKSHEET}' tab.`);
    }

    const worksheet = workbook.getWorksheet(HighlightCloudBlockerService.CLOUD_WORKSHEET);
    ...
    search_loop: for (let indexR = 1; indexR < worksheet.actualRowCount; indexR++) {
      for (let indexC = 1; indexC < worksheet.actualColumnCount; indexC++) {
        ...
        if (String(element.text) == HighlightCloudBlockerService.INDEX_TABLE_COL_NAME) {
    ...
    if (!found) {
      throw new Error(
        `The excel file is not valid. Missing ${HighlightCloudBlockerService.INDEX_TABLE_COL_NAME} table in worksheet ${HighlightCloudBlockerService.CLOUD_WORKSHEET}`,
      );
    }
  ...
  private async applyTag(blocker: CloudBlocker): Promise<boolean> {
    ...
    return o as node;`;
      ...
      const res: QueryResult = await HighlightCloudBlockerService.NEO4JAL.executeWithParameters(req, params);
