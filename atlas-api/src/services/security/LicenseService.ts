...
class LicenseService {
  private static INSTANCE: LicenseService = null;
  ...
  private constructor() {
    ...
    this.publicLoginKey = fs.readFileSync(path.join(appDir, "../config/keys/privateLicense.key"));
  }
  ...
  public static getInstance(): LicenseService {
    if (LicenseService.INSTANCE == null) {
  ...
  public async getActualLicense(): Promise<string> {
    ...
    // @ts-ignore
    if (!node.properties || node.properties[LicenseService.NODE_LICENSE_PROPERTY] == null) {
    ...
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    return String(node.properties[LicenseService.NODE_LICENSE_PROPERTY]);
  }
  ...
  private async deleteAllLoginConfigurationNode() {
    const delReq = `MATCH (n:${LicenseService.LOGIN_CONFIGURATION_NODE}) DETACH DELETE n;`;
  ...
  private async createLoginConfigurationNode(license: string) {
    ...
    const req = `MERGE (n:${LicenseService.LOGIN_CONFIGURATION_NODE}) 
        SET n.${LicenseService.NODE_LICENSE_PROPERTY}=$license`;
  ...
  private async getLoginConfigurationNode(): Promise<Node> {
    const reqSearch = `MATCH (n:${LicenseService.LOGIN_CONFIGURATION_NODE})  
